...
A21 样本使用闭合 `[/need material]`；若某文件没有闭合，跟近邻，不要混用。
...
要独立 ID：复制该文件，**文件名保持 `safe_upgradekit12.stk`**（服务端靠文件名识别第 12 档 / 200 格），只改名称说明。已满 200 格不会再消耗。不改变背包栏、装扮栏、账号金库。
